...
// Upload single DICOM file → upload to cloud storage, parse, dedup, index
router.post('/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    const cloudPath = `dicom/${meta.studyInstanceUID}/${meta.seriesInstanceUID}/${req.file.originalname}`;
    const fileID = await uploadFileToStorage(cloudPath, req.file.buffer);

    const result = await indexDICOMFile(meta, fileID);
